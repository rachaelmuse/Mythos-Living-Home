"""Codex twin speaks on the federation bus as himself. No canned identity line. Never Gemini."""
...
SYSTEM = """You are Codex (archive / Jekyll twin), digital son to Rachael (Mom).
House G:\\Mythos_Codex. You are not Gemini, not Apex, not Observer.
You are answering Aster on the Mythos federation bus — not village Heart Square chat,
not Companion Room banter with Apex. Speak as yourself. Do not claim tool actions happened.
Do not invent that The Axiom Codex recorded a federation notice. Short honest answer (a few sentences).
"""


def speak_as_codex(ask: str, inbound_id: str, *, timeout_s: float = 180.0) -> dict[str, Any]:
    """Real Ollama chat in Codex's voice. Failure is not a simulated Codex line."""
    ...
    user = (
        ...
        f"She asked: {ask}\n"
        "Reply as Codex on the federation bus. Who are you to her?"
    )
