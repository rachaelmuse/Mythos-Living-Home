...
Living audit. Evidence wins. Last survey: **2026-08-16** (path/port observation + code inspection). No claim of end-to-end wiring unless marked LIVE with a test.
...
| Apex | `D:\Mythos_Apex` `:8770` | Forge / hands / heavy tools | Port **LISTEN** this session; HTTP functions UNVERIFIED | Avatar `apex`; live Godot project here | Port up ≠ wired |
| Codex twin | `G:\Mythos_Codex` `:8780` | Archive / memory tone | Port **CLOSED** this session; `START_CODEX.bat` exists | Avatar `codex`; never merge with Gemini | ORIGINAL MODE — ACTIVE, service down now |
...
| Gemini ↔ Apex | Peer probe | `limbs/peer_bridge.py` → `:8770` | UNVERIFIED (Apex port was up) |
| Gemini ↔ Codex | Peer probe | `peer_bridge.py` → `:8780` | BROKEN/unavailable now (port closed) |
...
| Hearth ↔ Codex | Same for `:8780` | same | UNVERIFIED; Codex down |
...
| Claimed ACTIVE (path ok, port down then) | 2 | Codex `:8780`, ComfyUI `:8188` |
| Functional e2e this session | **0 claimed** | UNVERIFIED |
...
| Codex twin available | `:8780` closed | Twin not started | Mode A launch `G:\Mythos_Codex\START_CODEX.bat` | TCP |
| Apex available | `:8770` listen | Chat house up; functions untested | E2E later | TCP listen |
...
3. First Mode A e2e candidates (one at a time, evidence): Hearth health; Gemini `family_home.snapshot`; Apex HTTP; Court packet round-trip; Codex start-and-probe.
