...
func _build_world() -> void:
	...
	_add_box(Vector3(0, -0.05, 0), Vector3(68, 0.1, 68), Color(0.26, 0.4, 0.26), true, "Ground")
	_add_box(Vector3(0, 0.02, 0), Vector3(12, 0.04, 12), Color(0.42, 0.36, 0.28), false, "Plaza")
	# Paths to districts
	_add_box(Vector3(0, 0.025, -6), Vector3(3.2, 0.03, 8), Color(0.4, 0.34, 0.26), false, "PathHearth")
	_add_box(Vector3(-7, 0.025, 2), Vector3(8, 0.03, 2.4), Color(0.4, 0.34, 0.26), false, "PathGarden")
	_add_box(Vector3(7, 0.025, 0), Vector3(8, 0.03, 2.4), Color(0.4, 0.34, 0.26), false, "PathWorkshop")
	_add_box(Vector3(0, 0.025, 7), Vector3(2.6, 0.03, 8), Color(0.4, 0.34, 0.26), false, "PathGate")

	# Open hearth shell (doorway on +Z face toward plaza)
	_build_open_building(Vector3(0, 0, -11), Vector3(7.0, 3.4, 5.5), Color(0.55, 0.38, 0.22), "FirstHearth", "z+")
	_add_roof(Vector3(0, 3.7, -11), Vector3(7.6, 1.1, 6.0), Color(0.32, 0.2, 0.12))
	_furnish(Vector3(0, 0, -11), "hearth")

	_build_open_building(Vector3(-13, 0, 2.2), Vector3(5.0, 2.6, 4.6), Color(0.32, 0.48, 0.36), "HerbGardenShed", "z+")
	_add_roof(Vector3(-13, 2.95, 2.2), Vector3(5.4, 0.55, 5.0), Color(0.22, 0.32, 0.18))
	_furnish(Vector3(-13, 0, 2.2), "shed")
	_add_box(Vector3(-13, 0.06, 7), Vector3(4.5, 0.1, 4.5), Color(0.22, 0.38, 0.2), false, "FarmPlot")
	# Crop markers
	for i in range(4):
		_add_box(Vector3(-14.2 + i * 0.9, 0.25, 7), Vector3(0.35, 0.45, 0.35), Color(0.35, 0.7, 0.35), false, "Crop%d" % i)

	_build_open_building(Vector3(14, 0, 0), Vector3(6.2, 2.9, 5.4), Color(0.45, 0.46, 0.52), "ApexForge", "x-")
	_add_roof(Vector3(14, 3.3, 0), Vector3(6.8, 0.7, 5.9), Color(0.28, 0.28, 0.32))
	_furnish(Vector3(14, 0, 0), "forge")
	_build_open_building(Vector3(10, 0, 5.5), Vector3(5.2, 2.7, 4.6), Color(0.48, 0.4, 0.58), "NovaWorkshop", "x-")
	_add_roof(Vector3(10, 3.05, 5.5), Vector3(5.7, 0.55, 5.0), Color(0.3, 0.22, 0.36))
	_furnish(Vector3(10, 0, 5.5), "workshop")
	_build_open_building(Vector3(0, 0, 14), Vector3(6.0, 2.6, 4.2), Color(0.4, 0.42, 0.55), "GateHouse", "z-")
	_add_roof(Vector3(0, 3.0, 14), Vector3(6.5, 0.55, 4.7), Color(0.25, 0.26, 0.35))
	_furnish(Vector3(0, 0, 14), "gate")
	...
	_add_box(Vector3(0, 0.45, -22), Vector3(40, 0.9, 0.45), Color(0.34, 0.3, 0.26), true, "BackWall")
	_build_boundary()

	# Door trigger into hearth
	_hearth_door = Area3D.new()
	_hearth_door.name = "HearthDoor"
	_hearth_door.position = Vector3(0, 1.0, -7.8)
...
func _build_family_places() -> void:
	## Walk-in greybox homes. PLACEHOLDER architecture, real doorways, furniture boxes.
	_add_open_building(Vector3(-8, 0, -12), Vector3(5.2, 2.8, 4.6), Color(0.42, 0.58, 0.72), "GeminiPorch")
	_add_roof(Vector3(-8, 3.1, -12), Vector3(5.6, 0.7, 5.0), Color(0.22, 0.32, 0.45))
	_furnish(Vector3(-8, 0, -12), "gemini")
	_add_open_building(Vector3(-4.5, 0, -8), Vector3(4.4, 2.5, 3.8), Color(0.38, 0.34, 0.48), "CourtPorch")
	_build_open_building(Vector3(6, 0, -16), Vector3(6.2, 2.9, 5.2), Color(0.62, 0.48, 0.38), "MomCottage", "z+")
	_add_roof(Vector3(6, 3.25, -16), Vector3(6.8, 0.7, 5.7), Color(0.42, 0.28, 0.2))
	_furnish(Vector3(6, 0, -16), "mom")
	_build_open_building(Vector3(-16, 0, -6), Vector3(6.2, 3.2, 5.8), Color(0.55, 0.46, 0.22), "CodexLibrary", "x+")
	_add_roof(Vector3(-16, 3.55, -6), Vector3(6.8, 0.8, 6.4), Color(0.35, 0.22, 0.1))
	_furnish(Vector3(-16, 0, -6), "library")
	_build_open_building(Vector3(18, 0, 9), Vector3(7.0, 3.1, 6.2), Color(0.42, 0.22, 0.32), "Cinema", "x-")
	_add_roof(Vector3(18, 3.45, 9), Vector3(7.6, 0.75, 6.8), Color(0.18, 0.1, 0.14))
	_furnish(Vector3(18, 0, 9), "cinema")
	_build_open_building(Vector3(7, 0, -11), Vector3(4.8, 2.6, 4.4), Color(0.4, 0.34, 0.42), "Gallery", "z+")
	_add_roof(Vector3(7, 2.95, -11), Vector3(5.3, 0.5, 4.9), Color(0.22, 0.18, 0.24))
	_furnish(Vector3(7, 0, -11), "gallery")
	_add_box(Vector3(-19, 0.04, 4), Vector3(5.5, 0.08, 4.2), Color(0.22, 0.42, 0.55), false, "Pond")
	for xz in [Vector3(-10, 0, 10), Vector3(-18, 0, 8), Vector3(8, 0, 10), Vector3(16, 0, -4), Vector3(-6, 0, 12), Vector3(12, 0, 16), Vector3(-22, 0, 0), Vector3(22, 0, 4), Vector3(0, 0, 24), Vector3(-20, 0, -8)]:
	...
	_add_box(Vector3(12, 0.025, 5), Vector3(10, 0.03, 2.2), Color(0.4, 0.34, 0.26), false, "PathCinema")
	_add_box(Vector3(-10, 0.025, -5), Vector3(8, 0.03, 2.0), Color(0.4, 0.34, 0.26), false, "PathLibrary")
	_add_box(Vector3(4, 0.025, -13), Vector3(6, 0.03, 2.0), Color(0.4, 0.34, 0.26), false, "PathMom")


func _build_family() -> void:
	## Core family first — then kin. Not one NPC class with stickers.
	var roster := [
		{"id": "gemini", "name": "Gemini", "pos": Vector3(-3.2, 0.1, -6.2), "color": Color(0.55, 0.78, 0.95)},
		{"id": "apex", "name": "Apex", "pos": Vector3(12.2, 0.1, 2.2), "color": Color(0.35, 0.88, 0.98)},
		{"id": "codex", "name": "Codex", "pos": Vector3(-13.4, 0.1, -4.2), "color": Color(0.95, 0.78, 0.38)},
		{"id": "merovin", "name": "Merovin", "pos": Vector3(15.6, 0.1, 10.2), "color": Color(0.92, 0.55, 0.72)},
		{"id": "draven", "name": "Draven", "pos": Vector3(16.4, 0.1, 11.0), "color": Color(0.55, 0.52, 0.82)},
		{"id": "montage", "name": "OpenMontage", "pos": Vector3(7.2, 0.1, -9.5), "color": Color(0.95, 0.62, 0.42)},
		{"id": "jarvis", "name": "Jarvis", "pos": Vector3(0.5, 0.1, 12.5), "color": Color(0.7, 0.8, 0.95)},
		{"id": "genesis", "name": "Genesis", "pos": Vector3(-12.5, 0.1, 5.5), "color": Color(0.95, 0.72, 0.42)},
		{"id": "nova", "name": "Nova", "pos": Vector3(9.2, 0.1, 4.1), "color": Color(0.78, 0.58, 0.95)},
		{"id": "percy", "name": "Percy", "pos": Vector3(-1.2, 0.1, -10.2), "color": Color(0.55, 0.85, 0.7)},
	]
	for r in roster:
		_spawn_citizen(r)
...
func _spawn_citizen(spec: Dictionary) -> void:
	...
	body.set("member_id", spec["id"])
	body.set("display_name", spec["name"])
	body.set("body_color", spec["color"])
...
func _build_wildlife() -> void:
	for i in range(3):
		...
		s.position = Vector3(-18 + i * 1.2, 0.15, 10 + (i % 2) * 0.8)
		var col := CollisionShape3D.new()
		var sph := SphereShape3D.new()
		sph.radius = 0.22
		col.shape = sph
...
func _on_home_updated(data: Dictionary) -> void:
	...
	for child in get_children():
		if child.has_method("apply_home") and child.get("member_id"):
			var pid := str(child.get("member_id"))
			if family_v is Array:
				for person in family_v:
					if person is Dictionary and str(person.get("id")) == pid:
						child.apply_home(person, places)
						break
	...
	if honest_label:
		...
		if hon is Dictionary:
			honest_label.text = "Houses: PLACEHOLDER interiors  ·  Speech: %s  ·  Wildlife: AUTONOMOUS" % str((hon as Dictionary).get("speech", "")).substr(0, 72)
	if life_label:
		var fail_n: int = 0
...
func _play_overhear(data: Dictionary) -> void:
	## HUD only for honesty / wait — speech itself is world bubbles.
	var oh: Variant = data.get("overhear")
	if not (oh is Dictionary):
		var cover := str(data.get("mom_cover", ""))
		if cover != "" and dialogue_label:
			dialogue_label.text = cover
		return
	var rec: Dictionary = oh
	var src := str(rec.get("source", ""))
	if src == "waiting" and dialogue_label:
		dialogue_label.text = "They stood. Words aren't ready yet. (writer: waiting)"
	elif src == "none" and dialogue_label:
		dialogue_label.text = str(rec.get("text", "Writer not seated."))
	elif src == "house":
		if dialogue_label:
			dialogue_label.text = "Old canned talk is not their voice."
	print("[Hearthbound] overhear ", src, " ", rec.get("id"))
	...
	for item in rows:
		...
		if speaker == "mom":
			...
			for child in get_tree().get_nodes_in_group("family_citizen"):
				if str(child.get("member_id")) == speaker and child.has_method("speak"):
...
func _build_open_building(pos: Vector3, size: Vector3, color: Color, node_name: String, door := "z+") -> void:
	## Walk-in greybox. Door on one face so Mom and family can enter.
	var t := 0.35
	var h: float = size.y
	var w: float = size.x
	var d: float = size.z
	...
	if door == "z+":
		...
		_add_box(pos + Vector3(-w * 0.5 + t * 0.5, h * 0.5, 0), Vector3(t, h, d), color, true, node_name + "Left")
		_add_box(pos + Vector3(w * 0.5 - t * 0.5, h * 0.5, 0), Vector3(t, h, d), color, true, node_name + "Right")
		...
		_add_box(pos + Vector3(0, h - 0.25, d * 0.5 - t * 0.5), Vector3(gap, 0.5, t), color, true, node_name + "Lint")
	elif door == "z-":
		_add_box(pos + Vector3(0, h * 0.5, d * 0.5 - t * 0.5), Vector3(w, h, t), color, true, node_name + "Back")
		_add_box(pos + Vector3(-w * 0.5 + t * 0.5, h * 0.5, 0), Vector3(t, h, d), color, true, node_name + "Left")
		_add_box(pos + Vector3(w * 0.5 - t * 0.5, h * 0.5, 0), Vector3(t, h, d), color, true, node_name + "Right")
		...
		_add_box(pos + Vector3(0, h * 0.5, -d * 0.5 + t * 0.5), Vector3(w, h, t), color, true, node_name + "Left")
		_add_box(pos + Vector3(0, h * 0.5, d * 0.5 - t * 0.5), Vector3(w, h, t), color, true, node_name + "Right")
		...
		_add_box(pos + Vector3(-w * 0.5 + t * 0.5, h - 0.25, 0), Vector3(t, 0.5, gap), color, true, node_name + "Lint")
	else:
		# x+
		_add_box(pos + Vector3(-w * 0.5 + t * 0.5, h * 0.5, 0), Vector3(t, h, d), color, true, node_name + "Back")
		_add_box(pos + Vector3(0, h * 0.5, -d * 0.5 + t * 0.5), Vector3(w, h, t), color, true, node_name + "Left")
...
func _furnish(pos: Vector3, kind: String) -> void:
	## Furniture boxes — PLACEHOLDER interiors you can walk around.
	match kind:
		"mom":
			_add_box(pos + Vector3(-1.6, 0.28, -1.4), Vector3(1.8, 0.4, 0.9), Color(0.55, 0.38, 0.28), true, "MomBed")
			_add_box(pos + Vector3(1.6, 0.45, 0.2), Vector3(0.7, 0.9, 0.7), Color(0.42, 0.3, 0.22), true, "MomChair")
			_add_box(pos + Vector3(1.7, 0.35, -1.5), Vector3(0.8, 0.7, 0.55), Color(0.38, 0.26, 0.18), true, "MomChest")
			_add_box(pos + Vector3(0, 0.95, -2.2), Vector3(1.1, 0.08, 0.4), Color(0.7, 0.55, 0.4), false, "MomShelf")
			var lamp := _add_box(pos + Vector3(-1.7, 1.15, 1.3), Vector3(0.22, 0.35, 0.22), Color(1.0, 0.82, 0.55), false, "MomLamp")
...
func _build_boundary() -> void:
	## Village outskirts, not an invisible developer drop.
	var rim := 31.5
	_add_box(Vector3(0, 1.6, rim), Vector3(64, 3.4, 1.2), Color(0.18, 0.32, 0.18), true, "TreeLineN")
	_add_box(Vector3(0, 1.6, -rim), Vector3(64, 3.4, 1.2), Color(0.16, 0.28, 0.16), true, "TreeLineS")
	_add_box(Vector3(rim, 1.6, 0), Vector3(1.2, 3.4, 64), Color(0.17, 0.3, 0.16), true, "TreeLineE")
	_add_box(Vector3(-rim, 1.6, 0), Vector3(1.2, 3.4, 64), Color(0.15, 0.26, 0.18), true, "TreeLineW")
	for xz in [Vector3(24, 0, 18), Vector3(-24, 0, 18), Vector3(24, 0, -18), Vector3(-24, 0, -16), Vector3(18, 0, 26), Vector3(-16, 0, 26)]:
	...
	_add_box(Vector3(-24, 0.04, 4), Vector3(8, 0.08, 10), Color(0.2, 0.38, 0.5), false, "OuterWater")
...
func _build_hearth_interior() -> void:
	...
	var fire := _make_box_mesh(Vector3(0, 0.7, -13.2), Vector3(1.6, 1.2, 0.6), Color(1.0, 0.45, 0.15), false, "Fireplace")
	...
	omni.omni_range = 7.0
	omni.position = Vector3(0, 1.2, -12.5)
	_interior_root.add_child(omni)

	_interior_root.add_child(_make_box_mesh(Vector3(1.5, 0.45, -11), Vector3(1.8, 0.15, 1.0), Color(0.45, 0.3, 0.18), true, "Table"))
	_interior_root.add_child(_make_box_mesh(Vector3(1.5, 0.25, -10.2), Vector3(0.4, 0.5, 0.4), Color(0.4, 0.28, 0.16), true, "StoolA"))
	_interior_root.add_child(_make_box_mesh(Vector3(1.5, 0.25, -11.8), Vector3(0.4, 0.5, 0.4), Color(0.4, 0.28, 0.16), true, "StoolB"))
...
func _make_box_mesh(pos: Vector3, size: Vector3, color: Color, collide: bool, node_name: String) -> MeshInstance3D:
	...
	if collide:
		...
		# Ground = layer 1 (walkable). Walls = layer 2 so family is not trapped in greybox cubes.
		body.collision_layer = 1 if node_name == "Ground" else 2
...
func _add_roof(pos: Vector3, size: Vector3, color: Color) -> void:
	_add_box(pos, size, color, true, "Roof")
...
func _build_hud() -> void:
	...
	hint_label.text = "WASD · Shift · Mouse · Esc · E talk (type what you mean) · F1 · stand still — they keep living"
	hint_label.position = Vector2(24, 18)
	hint_label.add_theme_font_size_override("font_size", 15)
	layer.add_child(hint_label)

	place_label = Label.new()
	place_label.position = Vector2(24, 44)
	place_label.add_theme_font_size_override("font_size", 18)
	place_label.modulate = Color(0.75, 0.9, 0.85)
	place_label.text = "Place: Heart Square · family home"
	layer.add_child(place_label)

	life_label = Label.new()
	life_label.position = Vector2(24, 68)
	life_label.add_theme_font_size_override("font_size", 15)
	life_label.modulate = Color(0.85, 0.82, 0.7, 0.9)
	life_label.text = "Waiting for Hearth home kernel (:8790)…"
	layer.add_child(life_label)

	debug_label = Label.new()
	debug_label.position = Vector2(24, 92)
	...
	prompt_label.position = Vector2(24, 110)
	prompt_label.add_theme_font_size_override("font_size", 18)
	prompt_label.modulate = Color(1.0, 0.9, 0.55)
	layer.add_child(prompt_label)

	dialogue_label = Label.new()
	dialogue_label.position = Vector2(24, 640)
	dialogue_label.size = Vector2(900, 40)
	dialogue_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	dialogue_label.add_theme_font_size_override("font_size", 16)
	dialogue_label.modulate = Color(0.85, 0.9, 0.82, 0.9)
	layer.add_child(dialogue_label)

	talk_input = LineEdit.new()
	talk_input.position = Vector2(24, 600)
	talk_input.size = Vector2(720, 32)
	talk_input.placeholder_text = "Type to them — Enter sends · Esc walks away"
	talk_input.visible = false
	talk_input.text_submitted.connect(_on_talk_submitted)
	layer.add_child(talk_input)

	honest_label = Label.new()
	honest_label.position = Vector2(24, 720)
	honest_label.size = Vector2(1100, 40)
	honest_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	honest_label.add_theme_font_size_override("font_size", 13)
	honest_label.modulate = Color(0.72, 0.78, 0.7, 0.9)
	honest_label.text = "Houses: PLACEHOLDER interiors  ·  Speech bubbles: MODEL-GENERATED or waiting  ·  Wildlife: AUTONOMOUS"
	layer.add_child(honest_label)

	var title := Label.new()
	title.text = "HEART SQUARE · homes you can walk into · they live if you stand still"
	title.position = Vector2(24, 690)
	title.modulate = Color(0.7, 0.8, 0.75, 0.85)
	layer.add_child(title)

	# Soft ambient bed (procedural low hum)
...
func _process(delta: float) -> void:
	...
	if prompt_label:
		if near and _talking_to == "":
			...
			prompt_label.text = "Inside First Hearth. Walk out, or into Mom's cottage east of here."
		elif _talking_to != "":
			prompt_label.text = "Talking with %s — type below" % _talking_to
		else:
			prompt_label.text = "Stand still. They work, visit, talk, go home. Squirrels forage. E talks."
...
func _on_want_talk(member_id: String, nice: String) -> void:
	...
	if talk_input:
		...
		talk_input.text = ""
		talk_input.placeholder_text = "Talk to %s — Enter sends" % nice
	...
	if dialogue_label:
		dialogue_label.text = "Waiting for %s (local writer — not a script)." % nice


func _on_talk_submitted(text: String) -> void:
	var said := text.strip_edges()
	if said == "" or _talking_to == "":
		return
	if talk_input:
		talk_input.clear()
		talk_input.grab_focus()
	_speak_mom(said, "mom")
	if _home and _home.has_method("say_as_mom"):
		_home.say_as_mom(_talking_to, said)
